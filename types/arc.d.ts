 ...
 * @property {number} [size=50] - Maximum cache size
 */

/**
 * ARC (Adaptive Replacement Cache) implementation
 * @param {ArcOptions} [options={}] - Configuration options
 * @returns {ArcCache} - New ARC cache instance
 */
export function arc(options = {}) {
  return new ArcCache(options.size || 50);
}
